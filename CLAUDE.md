...
infra/
  ...
  packer/                   # Packer template for pre-baked runner AMI (AL2023)
    jit-runner.pkr.hcl      # amazon-ebs source; community AMI catalog publishing
    variables.pkr.hcl       # runner_version, aws_region, ami_regions, instance_type, extra_script, ami_name_prefix, subnet_id
    scripts/setup-runner.sh # Base provisioning: deps, runner user, runner agent, marker file
...
make ami.build           # Build pre-baked runner AMI in us-east-2
...
- **AMI build CI**: `.github/workflows/ami-build.yml` — workflow_dispatch (inputs: `runner_version`, `extra_script`, `distribute`) and auto-trigger on push to `infra/packer/**`. Uses OIDC role assumption via `AMI_BUILD_ROLE_ARN` secret. Distribute copies AMI to US, EU, and SA regions.

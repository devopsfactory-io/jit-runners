...
- **Branch name**: Use user-provided name if given. Otherwise derive from the commit message: e.g. `feat/scope-short-desc`, `fix/short-desc`, `enhance/improve-x`, `ci/update-workflow`, or `(deps)/lambda-go-mod` (slug from the first line of the conventional commit). These patterns align with [.github/labeler.yml](.github/labeler.yml) so the labeler auto-applies PR labels (feature, bug, enhancement, github-actions, dependencies). For breaking changes, include `!` in the branch name to get the `breaking-change` label. Prefer lowercase, hyphens; avoid spaces.
...
- Create PR: `gh pr create` using the repository's [pull request template](.github/pull_request_template.md). Populate all sections (see **PR body** below).
...
The repo uses [.github/pull_request_template.md](.github/pull_request_template.md). When creating the PR, populate:
...
- **Checklist** – Mark as appropriate: breaking changes (Yes/No/N/A), docs updated, tests added/updated (`make lambda.test` passes).
- **AI Summary** – For AI reviewers; fill from changed files and context:
  - **Scope:** e.g. `lambda/webhook`, `lambda/scaleup`, `infra/terraform`, `docs`
  - **Type:** `feat` | `fix` | `docs` | `refactor`
  - **Key files/areas:** paths from `git diff --name-only` or conversation
